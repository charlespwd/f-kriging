module error
   ...
   double precision, allocatable :: FtRinvFinv(:,:)
   integer, parameter :: MSE_MODE_LOPHAVEN = 1
   integer, parameter :: MSE_MODE_ARTHUR = 2
   integer, parameter :: MSE_MODE_MARTIN = 3
   contains
!! 
! arthur_mse
...
double precision function arthur_mse(sigma2, F, Rinv, rx, D, Ns, fdim)
   ...
   term3 = term3 * term4
   arthur_mse = abs(sigma2 * (term1 - term2 + term3))
end function 
...
double precision function lophaven_mse(sigma2, fx, rx, F, R, Rinv, ns, fdim, iter)
   use LA_PRECISION, only:WP=>DP
   use F95_LAPACK, only: LA_GESVX
   use matrix, only: eye
   implicit none
   integer, intent(in) :: ns, fdim, iter
   double precision, intent(in) :: sigma2
   double precision, intent(in) :: fx(fdim, 1), rx(ns, 1)
   double precision, intent(in) :: F(ns, fdim), R(ns, ns)
   double precision, intent(in) :: Rinv(ns, ns)
   ...
   double precision :: utFtRinvFinv(1, fdim)
   ...
   ! The following block is present for code optimization. The inverse of
   ! FtRinvF is independent of the new snapshot location. For this
   ! reason, I include the following deallocate/allocate/calculate block so that
   ! I do not need to perform a matrix inversion at every MSE calculation and to
   ! maintain the modularity of the MSE function. The matrix inversion is only
   ! performed for the first x_new.
   if (iter == 1) then
      if(allocated(FtRinvFinv)) then
         deallocate(FtRinvFinv) 
      end if
      allocate(FtRinvFinv(fdim, fdim)) 
      ! invert FtRinvF
